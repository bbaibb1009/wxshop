...
function enableInput()
{
	...
	var index  = arguments[1];
	$(chkBox).parent().children("input[type='text'],select").removeAttr("disabled");
}
...
function disableInput()
{
	var chkBox = arguments[0];
	var index  = arguments[1];
	$(chkBox).parent().children("input[type='text'],select").attr("disabled","disabled");
}
...
function showSubBtns()
{
	var $this = arguments[0];
	var val = $($this).val();
	var childrenTr = $($this).parent().parent().next().next().nextUntil('.trNotSubBtn');
	if(val=='')
	{
		$(childrenTr).show();
	}
	...
	{
		$(childrenTr).hide();
		$(childrenTr).children().children("input[type='checkbox']").removeAttr("checked");
		$(childrenTr).children().children("input[type='text'],select").attr("disabled","disabled");
	}
		...
		[
			{
				...
				"sub_button": 
				[
					{"type": "view","name": "下载","url": "http://info.oilchem.net/lzsht/index.html","sub_button": []},
					{"type": "view","name": "优势","url": "http://mp.weixin.qq.com/s?__biz=MzA3MDU3OTgzMA==&mid=201343703&idx=1&sn=6f9d66051dbf12b91de5291774fa30f4#rd","sub_button":[]}
				]
			},
			{	
				"type": "view",
				"name": "供求",
				"url": "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx2ba0ce17184b93b7&redirect_uri=http%3A%2F%2Finfo.oilchem.net%2Fweixin%2FtoZhongjianYeMian&response_type=code&scope=snsapi_base&state=STATE#wechat_redirect",
				"sub_button":[]
			},
			{
				"type": "view",
				"name": "短讯通",
				"url": "http://img.oilchem.net/2014/lzdxt/",
				"sub_button":[]
			}
		]
	}
	*/
	...
	if($(needBtn).length>0)
	{
		...
		$(needBtn).each(function(index){
			...
			var complexBtn = {};
			var complexType = $("select[name='selType_"+chkIndex+"']").val();
			var complexName = $("input[name='iptName_"+chkIndex+"']").val();
			complexBtn.name = complexName;
			if(complexType=='')
			{
				var checkSubChks = $("input[name='checkSubChk_"+chkIndex+"']:checked");
			...
			else if(complexType=='view')
			{
				...
				complexBtn.url  = $("input[name='iptUrl_"+chkIndex+"']").val();
				complexBtn.sub_button = [];
			}
			else if(complexType=='click')
			{
				complexBtn.type = complexType;
				complexBtn.key  = $("input[name='iptKey_"+chkIndex+"']").val();
				complexBtn.sub_button = [];
			}
			else
			{
...
function needBtn()
{
	...
	if(!$($this).prop("checked"))
	{
		...
		var childrenTr = $($this).parent().parent().next().next().next().next().nextUntil('.trNotSubBtn');
		$(childrenTr).children().children("input[type='checkbox']").removeAttr("checked");
		$(childrenTr).children().children("input[type='text'],select").attr("disabled","disabled");
	}
	else
...
function initSubButton()
{
	...
	$(button).each(function(index){
		...
		var checkBtn = $("input[name='checkBtn_"+count+"']").attr('checked','checked');
...
function initSubBtns()
{
	...
	$(sub_button).each(function(index){
		...
		var type = $(subbtn).prop("type")?'':$(subbtn).prop("type");
		
		var url = subbtn.url;
		var key = subbtn.key;
		$("#checkSubChk_"+parent_count+"_"+count).attr("checked","checked").removeAttr("disabled");
		$("#iptSubName_"+parent_count+"_"+count).val(name).removeAttr("disabled");		
		$("#selSubType_"+parent_count+"_"+count).val(type).removeAttr("disabled");
		$("#subIptUrl_"+parent_count+"_"+count).val(url).removeAttr("disabled");
		$("#subIptKey_"+parent_count+"_"+count).val(key).removeAttr("disabled");
