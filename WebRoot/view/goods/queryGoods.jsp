...
<html>
  ...
  <body>
  	...
  	<div class="content">
  	<f:form action="${pageContext.request.contextPath}/goods/queryGoods/${command.wgsWcsId}" onsubmit="return false;">
  	<f:hidden path="wgsId"/>
    ...
    <input type="button" value="删除" onclick="delChk('wgsIds', '${pageContext.request.contextPath}/goods/delGoods/${fenlei.wgfId}');"/>
 	<table cellpadding="0" cellspacing="0" border="1" width="100%">
    	<tr>
    		<th width="1%"><input type="checkbox" onclick="chkall(this.checked, 'wgsIds');"/></th>
    		<th width="10%">商品名称</th>
    		<th width="8%">商品分类</th>
    		<th width="8%">标准销售价</th>
    		<th width="3%">零售价</th>
    	...
    	<c:forEach var="goods" items="${pageResult.resultList}">
	    	<tr>
	    		...
	    		<td>
	    			<a href="javascript:toUpd('wgsId', ${goods.WGS_ID}, '${pageContext.request.contextPath}/goods/toUpdGoods')">修改</a>
	    			<a href="javascript:toUpd('wgsId', ${goods.WGS_ID}, '${pageContext.request.contextPath}/goods/toInGoods')">入库</a>
	    			<c:if test="${goods.WGS_KUCUN>0}">
	    			<a href="javascript:toUpd('wgsId', ${goods.WGS_ID}, '${pageContext.request.contextPath}/goods/toOutGoods')">出库</a>
	    			<a href="javascript:toUpd('wgsId', ${goods.WGS_ID}, '${pageContext.request.contextPath}/goods/toBadGoods')">坏账</a>
	    			</c:if>
	    		</td>																													
	    	</tr>
    	</c:forEach>
