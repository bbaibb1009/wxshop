...
<html>
  ...
  <body>
  	...
  	<div class="content">
  	<f:form action="${pageContext.request.contextPath}/goods/queryGoodsFenlei/${command.wgfWcsId}" onsubmit="return false;">
  	<f:hidden path="wgfId"/>
  	<input type="button" value="添加" onclick="window.location.href='${pageContext.request.contextPath}/goods/toAddGoodsFenlei/${command.wgfWcsId}';"/>
 	<input type="button" value="删除" onclick="delChk('wgfIds', '${pageContext.request.contextPath}/goods/delGoodsFenlei/${command.wgfWcsId}');"/>
 	<table cellpadding="0" cellspacing="0" border="1" width="100%">
    	...
    	<c:forEach var="goodsfeilei" items="${pageResult.resultList}">
	    	<tr>
	    		...
	    		<td>${goodsfeilei.WSA_NAME}</td>
	    		<td><fmt:formatDate value="${goodsfeilei.WGF_REGISTDATE}" type="date"/></td>
	    		<td>
	    			<a href="${pageContext.request.contextPath}/goods/queryGoods/${goodsfeilei.WGF_ID}">查看所有商品</a>
	    			<a href="javascript:toUpd('wgfId', ${goodsfeilei.WGF_ID}, '${pageContext.request.contextPath}/goods/toUpdGoodsFenlei')">修改</a>
	    		</td>																													
	    	</tr>
    	</c:forEach>
    </table>
